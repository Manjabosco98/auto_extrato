...
class Sicoob(BankHandler):
    ...
    @layout("SICOOB", "SISTEMA DE COOPERATIVAS DE CRÉDITO DO BRASIL", " EXTRATO CONTA CORRENTE ") #, "PLATAFORMA DE SERVIÇOS FINANCEIROS DO SICOOB - SISBR"
    ...
    # @layout("SISTEMA DE COOPERATIVAS DE CRÉDITO DO BRASIL")
    # def layout2(self, pdf):
    #     indice = next((i for i, item in enumerate(pdf) if "DEVOLUÇÃO" in item), None)
    #     pdf = pdf[10:indice]
    #     pdf = [item for item in pdf if not any(texto in item for texto in ["DATA HISTÓRICO Valor", "SALDO ", "INTEGRALIZAÇÃO"])]
